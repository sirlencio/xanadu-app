    ...
    <div class="w-full max-w-screen-lg mx-auto p-4 mt-14">
        <div class="flex flex-col md:flex-row items-center">
            <div class="flex-shrink-0 md:mr-8 bg-gray-200 rounded-lg overflow-hidden border-2">
                <img src="{{ $serie->poster }}" alt="{{ $serie->titulo }}" class="w-64 h-auto mb-4">
            </div>
            <div class="flex flex-col md:flex-1">
                ...
                <div class="mt-4">
                    ...
                    @else
                        <p class="text-lg text-gray-300 mb-4">{{ $serie->sinopsis }}</p>
                    @endif
                </div>
        ...
        @auth
            <div class="flex justify-center flex-col gap-10 md:flex-row items-center mt-10">
                ...
                @if (auth()->user()->watchlist != null &&
                        auth()->user()->watchlist->series != null &&
                        auth()->user()->watchlist->series->contains($serie->id))
                    <form action="{{ route('watchlist.remove', ['type' => 'serie', 'id' => $serie->id]) }}"
                        method="POST">
                        @csrf
                        <button type="submit"
                ...
                @else
                    <form action="{{ route('watchlist.add', ['type' => 'serie', 'id' => $serie->id]) }}" method="POST">
                        @csrf
                        <button type="submit"
                ...
                <!-- Boton Favoritos-->
                @if (auth()->user()->seriesFavoritas != null &&
                        auth()->user()->seriesFavoritas->contains($serie->id))
                    <form action="{{ route('favoritos.remove', ['type' => 'serie', 'id' => $serie->id]) }}"
                        method="POST">
                        @csrf
                        <button type="submit"
                            class="text-white border focus:outline-none font-medium rounded-lg text-base px-5 py-2.5 text-center inline-flex items-center mr-2 mb-2 border-blue-500 bg-blue-500">
                ...
                @else
                    <form action="{{ route('favoritos.add', ['type' => 'serie', 'id' => $serie->id]) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="border font-medium rounded-lg text-base px-5 py-2.5 text-center inline-flex items-center mr-2 mb-2 border-blue-500 text-blue-500">
                            <svg viewBox="2 2 20 20" class="w-5 h-5 mr-2 -ml-1" fill="none"
                ...
                <!-- Boton Vistos-->
                @if (auth()->user()->seriesVistas != null &&
                        auth()->user()->seriesVistas->contains($serie->id))
                    <form action="{{ route('vistos.remove', ['type' => 'serie', 'id' => $serie->id]) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="text-white border focus:outline-none font-medium rounded-lg text-base px-5 py-2.5 text-center inline-flex items-center mr-2 mb-2 border-blue-500 bg-blue-500">
                ...
                @else
                    <form action="{{ route('vistos.add', ['type' => 'serie', 'id' => $serie->id]) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="border font-medium rounded-lg text-base px-5 py-2.5 text-center inline-flex items-center mr-2 mb-2 border-blue-500 text-blue-500">
                            <svg viewBox="0 0 24 24" class="w-5 h-5 mr-2 -ml-1" fill="none"
